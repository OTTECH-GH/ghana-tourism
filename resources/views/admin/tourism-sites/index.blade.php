...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.tourism-sites.create') }}" class="bg-ghana-green text-white px-4 py-2 rounded-lg hover:bg-primary-700 text-sm">Add New Site</a>
</div>
        ...
        <tbody class="divide-y">
            @foreach($sites as $site)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        @if($site->is_featured) <span class="text-ghana-green font-bold">Yes</span> @else <span class="text-gray-400">No</span> @endif
                    </td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded text-xs {{ $site->is_active ? 'bg-green-50 text-ghana-green' : 'bg-red-100 text-red-700' }}">{{ $site->is_active ? 'Active' : 'Inactive' }}</span>
                    </td>
                    <td class="px-4 py-3">
