...
<div class="mb-6">
    <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-wrap gap-3">
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Search users..." class="rounded-lg border-gray-300 focus:ring-emerald-500">
        <select name="role" onchange="this.form.submit()" class="rounded-lg border-gray-300 focus:ring-emerald-500">
        ...
        <button type="submit" class="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700">Filter</button>
    </form>
</div>
        ...
        <tbody class="divide-y">
            @foreach($users as $user)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded text-xs {{ $user->status === 'active' ? 'bg-emerald-100 text-emerald-700' : ($user->status === 'suspended' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700') }}">{{ ucfirst($user->status) }}</span>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex gap-2">
                            ...
                            @else
                                <form action="{{ route('admin.users.update-status', $user) }}" method="POST">@csrf @method('PATCH')<input type="hidden" name="status" value="active"><button class="text-emerald-600 text-xs hover:text-emerald-800">Activate</button></form>
                            @endif
                        </div>
                    </td>
                </tr>
