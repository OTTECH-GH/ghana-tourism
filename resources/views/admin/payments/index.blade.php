...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    <div class="bg-white rounded-xl p-4 shadow-sm"><div class="text-sm text-gray-500">Total Revenue</div><div class="text-2xl font-bold text-emerald-600">GHS {{ number_format($totalRevenue, 2) }}</div></div>
        ...
        <tbody class="divide-y">
            @foreach($payments as $p)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3"><span class="px-2 py-1 rounded text-xs {{ $p->status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-yellow-100 text-yellow-700' }}">{{ ucfirst($p->status) }}</span></td>
