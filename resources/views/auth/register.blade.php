    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <select id="role" name="role" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                ...
                <option value="hotel_admin" {{ old('role') === 'hotel_admin' ? 'selected' : '' }}>Hotel Owner</option>
                <option value="driver" {{ old('role') === 'driver' ? 'selected' : '' }}>Driver</option>
                <option value="tour_guide" {{ old('role') === 'tour_guide' ? 'selected' : '' }}>Tour Guide</option>
                <option value="transport_company" {{ old('role') === 'transport_company' ? 'selected' : '' }}>Transport Company</option>
            </select>
