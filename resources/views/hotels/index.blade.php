...
<div class="bg-gradient-to-br from-ghana-green via-primary-700 to-primary-900 text-white py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p class="text-ghana-gold font-medium text-sm tracking-wide uppercase mb-2">Accommodation</p>
        <h1 class="text-4xl font-display font-bold">Hotels in Ghana</h1>
        <p class="text-green-200 mt-2">Find and book the best hotels near your favorite tourism sites.</p>
    </div>
</div>

<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('hotels.index') }}" method="GET" class="bg-white rounded-xl p-4 shadow-sm mb-8 flex flex-wrap gap-4">
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Search hotels..." class="flex-1 min-w-[200px] rounded-lg border-gray-300 focus:ring-ghana-green">
        ...
        <select name="stars" class="rounded-lg border-gray-300 focus:ring-ghana-green" onchange="this.form.submit()">
            <option value="">All Ratings</option>
            @for($i = 1; $i <= 5; $i++)
                <option value="{{ $i }}" {{ request('stars') == $i ? 'selected' : '' }}>{{ $i }}+ Stars</option>
            @endfor
        </select>
        <button type="submit" class="bg-ghana-green text-white px-6 py-2 rounded-lg hover:bg-primary-700">Filter</button>
    </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($hotels as $hotel)
            <a href="{{ route('hotels.show', $hotel) }}" class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition group">
                <div class="h-48 bg-gradient-to-br from-blue-400 to-primary-700 flex items-center justify-center">
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="font-bold text-gray-800 group-hover:text-ghana-green">{{ $hotel->name }}</h3>
                            <p class="text-sm text-gray-500">{{ $hotel->region->name }}</p>
                        </div>
                        <div class="text-ghana-gold text-sm">
                    ...
                    @if($hotel->rooms->count() > 0)
                        <div class="mt-3 flex justify-between items-center">
                            <span class="text-ghana-green font-bold">From GHS {{ number_format($hotel->rooms->min('price_per_night'), 2) }}/night</span>
