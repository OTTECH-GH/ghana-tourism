...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        ...
        <form action="{{ route('search') }}" method="GET" class="flex flex-wrap gap-4">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Search..." class="flex-1 min-w-[200px] rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500">
            <select name="category" class="rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500">
                <option value="">All Categories</option>
                @foreach($categories as $cat)
                    <option value="{{ $cat->id }}" {{ request('category') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                @endforeach
            </select>
            <select name="region" class="rounded-lg border-gray-300 shadow-sm focus:ring-emerald-500">
                <option value="">All Regions</option>
                @foreach($regions as $reg)
                    <option value="{{ $reg->id }}" {{ request('region') == $reg->id ? 'selected' : '' }}>{{ $reg->name }}</option>
                @endforeach
            </select>
            <button type="submit" class="bg-emerald-600 text-white px-6 py-2 rounded-lg hover:bg-emerald-700">Search</button>
        </form>
    </div>
            ...
            @foreach($sites as $site)
                <a href="{{ route('tourism.show', $site) }}" class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition group">
                    ...
                    <div class="p-4">
                        ...
                        <span class="text-emerald-600 font-bold mt-2 inline-block">GHS {{ number_format($site->entry_fee, 2) }}</span>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    @else
        <div class="text-center py-16">
            ...
            <a href="{{ route('tourism.index') }}" class="text-emerald-600 hover:text-emerald-700 mt-4 inline-block">Browse all sites &rarr;</a>
        </div>
    @endif
</div>
@endsection
