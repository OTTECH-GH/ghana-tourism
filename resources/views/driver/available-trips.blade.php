                ...
                <div>
                    ...
                    <div class="mt-2">
                        @foreach($trip->destinations as $d)
                            <span class="bg-ghana-green/5 text-ghana-green px-2 py-1 rounded text-xs mr-1">{{ $d->destination_name }}</span>
                        @endforeach
                    </div>
                    <p class="text-sm text-gray-500 mt-2">{{ $trip->trip_date->format('M d, Y') }} at {{ $trip->trip_time }} &middot; {{ $trip->passengers }} passengers</p>
                </div>
                <div class="text-right">
                    <div class="text-xl font-bold text-ghana-green">GHS {{ number_format($trip->estimated_amount, 2) }}</div>
                    <form action="{{ route('driver.accept-trip', $trip) }}" method="POST" class="mt-2">
                        @csrf @method('PATCH')
                        <button class="bg-ghana-green text-white px-6 py-2 rounded-lg hover:bg-primary-700">Accept Trip</button>
                    </form>
                </div>
            </div>
        </div>
